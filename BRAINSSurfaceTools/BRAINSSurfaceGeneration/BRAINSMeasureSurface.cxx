...
int main( int argc, char **argv )
{
  ...
  vtkShortArray *labelData = vtkShortArray::SafeDownCast( surfaceData->GetCellData()->GetAbstractArray( arrayName.c_str() ) );
  ...
  vtkFloatArray *curvature = vtkFloatArray::SafeDownCast( surfaceData->GetPointData()->GetAbstractArray("Mean_Curvature") );
  if (curvature==NULL)
  {
      std::cout<<"curvature is empty"<<std::endl;
      return EXIT_FAILURE;
  }
  vtkFloatArray *thickness = vtkFloatArray::SafeDownCast( surfaceData->GetPointData()->GetAbstractArray("corticalThickness") );
    if (thickness==NULL) {
  ...
  if ( testDepth )
    ...
    for ( unsigned int i = 1; i < (arraySize+1); i++ )
      ...
      if ((( fabs(gyralDepthResults[i-1]) - fabs( gyralThickness->GetValue(i) )) / fabs( gyralThickness->GetValue(i) )) >= 0.001 )
        {

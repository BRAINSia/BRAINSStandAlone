...
void IntensityMotionCheckPanel::f_overallInterlaceWiseCheck()
{
  ...
  _r_InterlaceWiseCheck = num_InterlaceWiseCheckExc/(qcResult.GetIntensityMotionCheckResult().size()-num_SliceWiseCheckExc);
...
void IntensityMotionCheckPanel::f_overallGradientWiseCheck()
{
  ...
  _r_GradWiseCheck = num_GradientWiseCheckExc/(qcResult.GetIntensityMotionCheckResult().size()-num_SliceWiseCheckExc-num_InterlaceWiseCheckExc);
...
void IntensityMotionCheckPanel::Set_Original_ForcedConformance_Mapping()
{
  ...
  for ( unsigned int i = 0 ; i < GetDwiOutputImage()->GetVectorLength() ; i++ )
    ...
    for ( unsigned int k = 0 ; k < qcResult.GetIntensityMotionCheckResult().size(); k++ )
      {
      if ( i == static_cast<unsigned int>(qcResult.GetIntensityMotionCheckResult()[k].QCIndex) )
        {
        Bing.push_back( k );

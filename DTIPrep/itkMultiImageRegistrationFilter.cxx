...
::AffineRegistration()
{
   ...
   for(unsigned int i=0; i<m_NumberOfImages; i++)
   {
      ...
      ImageType::SizeType size = m_ImagePyramidArray[i]->GetOutput(m_ImagePyramidArray[0]->GetNumberOfLevels()-1)->GetLargestPossibleRegion().GetSize();
      ImageType::IndexType startIndex = m_ImagePyramidArray[i]->GetOutput(m_ImagePyramidArray[0]->GetNumberOfLevels()-1)->GetLargestPossibleRegion().GetIndex();
      ...
      m_ImagePyramidArray[i]->GetOutput(m_ImagePyramidArray[0]->GetNumberOfLevels()-1)->TransformContinuousIndexToPhysicalPoint(centerIndex, center);
...
{ 
  ...
  m_Registration->SetTransformParametersLength( static_cast<int>( pow( static_cast<double>(m_BsplineInitialGridSize+SplineOrder),
            static_cast<int>(Dimension))*Dimension*m_NumberOfImages ));
  ...
  for(unsigned int i=0; i<m_NumberOfImages; i++)
  {
     ...
     m_BsplineInitializerArray[i]->SetImage(m_ImagePyramidArray[i]->GetOutput(m_ImagePyramidArray[i]->GetNumberOfLevels()-1));

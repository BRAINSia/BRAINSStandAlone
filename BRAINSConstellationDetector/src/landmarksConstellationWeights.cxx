...
// 2- The results of the BCD is compared with the training landmarks files. For each landmark point, the average and the STD (standard deviation) of error is calculated.

// 3- Resultant STD values are used to compute the wights such that the weight of a landmark point, which has the lowest std, is mapped to ONE, and the weights of the other landmarks are calculated correspondingly. The landmark with the highest STD has the lowest weight that is close to zero.
...
int main( int argc, char *argv[] )
{
       ...
       || ( llsModel.compare("") == 0 ) )
    {
        ...
        << "output weight list filename." << "\nAlso, the \"LLSModel\" and the \"templateModel\" of the target landmarks list should be defined." << std::endl;
    ...
    */
    ...
    // the weight of a landmark point, which has the lowest std, should be mapped to ONE, and weights of the other landmarks should be calculated correspondingly
    double Margin = 0.05;

                            ...
                            Image4DType::Pointer &img4D)
{
  ...
  for(vectorIndex[2] = 0; vectorIndex[2] < static_cast<VectorImageType::IndexType::IndexValueType>( size3D[2] ); ++vectorIndex[2])
    {
    index4D[2] = vectorIndex[2];
    for(vectorIndex[1] = 0; vectorIndex[1] < static_cast<VectorImageType::IndexType::IndexValueType>( size3D[1] ); ++vectorIndex[1])
      {
      index4D[1] = vectorIndex[1];
      for(vectorIndex[0] = 0; vectorIndex[0] < static_cast<VectorImageType::IndexType::IndexValueType>( size3D[0] ); ++vectorIndex[0])
        {
        index4D[0] = vectorIndex[0];
        const VectorImageType::PixelType pixel = img->GetPixel(vectorIndex);
        for(index4D[3] = 0; index4D[3] < static_cast<VectorImageType::IndexType::IndexValueType>( size4D[3] ); ++index4D[3])
          {

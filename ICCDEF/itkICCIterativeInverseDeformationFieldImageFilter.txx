...
ICCIterativeInverseDeformationFieldImageFilter<TInputImage, TOutputImage>::ICCIterativeInverseDeformationFieldImageFilter()
{
...
void 
ICCIterativeInverseDeformationFieldImageFilter<TInputImage, TOutputImage>
::ThreadedComputeInverse(InputImageConstPointer& inputPtr, OutputImagePointer& outputPtr, const ThreadRegionType &regionToProcess, int)
{
    ...
    while( !OutputIt.IsAtEnd() )
	  ...
	  const float fBasePnt1 = static_cast < float >(index[0]) / fnx;
        ...
        do
          ...
          while (((vcl_abs (fDelta1) > fEpsilon1) || (vcl_abs (fDelta2) > fEpsilon2) || (vcl_abs (fDelta3) > fEpsilon3)) && (iIteration < current_check_level));
          perturbation *= PerturbationScaleFactor;//Reduce stepsize in attemp to get out of local minimum. // 0.5 decreases too quickly
          current_check_level = current_check_level + OnePercentIncrementOfMaxIterations;
          }
        while (((vcl_abs (fDelta1) > fEpsilon1) || (vcl_abs (fDelta2) > fEpsilon2) || (vcl_abs (fDelta3) > fEpsilon3)) && (iIteration < m_NumberOfIterations));
        }

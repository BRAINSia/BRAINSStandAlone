  ...
  // Set up the demons filter
    ...
    if (command.useConsistentLandmark)
    {
       ...
       else
       {
          ...
          filter->SetMovingLandmark(dynamic_cast< LandmarkSpatialObjectType * >(movingLandmarkReader->GetGroup().GetPointer()));
        #endif
          ...
          fixedVolume = itkUtil::OrientImage<TRealImage>(fixedVolume,itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
          typename  TRealImage::Pointer movingVolume
                   = itkUtil::ReadImage<TRealImage>( command.movingVolume.c_str () );
          movingVolume = itkUtil::OrientImage<TRealImage>(movingVolume,itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);

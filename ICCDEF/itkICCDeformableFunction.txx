...
// std::cout<<"Function!"<<std::endl; 
  ...
  if(this->GetUseConsistentIntensity())
  {
    ...
    if(this->GetMovingImageMask() && this->GetFixedImageMask() )
    {
    	...
    	IterationMaskImageType dit(m_MovingMaskImageWarper->GetOutput(),m_MovingMaskImageWarper->GetOutput()->GetRequestedRegion());
...
//	m_SimilarityWeight = 0.1; //40.0/256.0;
	...
	typename TDeformationField::PixelType pixel;
        ...
        if(this->GetMovingImageMask() && this->GetFixedImageMask() )
        {
          for(def12Iter.GoToBegin(), fit.GoToBegin(),mit.GoToBegin(), xIter.GoToBegin(), yIter.GoToBegin(), zIter.GoToBegin();
              !def12Iter.IsAtEnd(); ++def12Iter,++fit,++mit,++xIter,++yIter,++zIter)
          {
              ...
              if(this->GetFixedImageMask()->IsInside(fixedPoint) &&  (m_MovingMaskImageWarper->GetOutput()->GetPixel(index) > 0) )
              {
              ...
              else if(!this->GetFixedImageMask()->IsInside(fixedPoint) &&  (m_MovingMaskImageWarper->GetOutput()->GetPixel(index) > 0) )
              {
                pixel[0] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (mit.Get()-m_BackgroundFilledValue) * xIter.Get() ;
                pixel[1] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (mit.Get()-m_BackgroundFilledValue) * yIter.Get() ;
                pixel[2] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (mit.Get()-m_BackgroundFilledValue) * zIter.Get() ;
              }
              else if(this->GetFixedImageMask()->IsInside(fixedPoint) &&  (m_MovingMaskImageWarper->GetOutput()->GetPixel(index) <= 0) )
              {
                pixel[0] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_BackgroundFilledValue-fit.Get()) * xIter.Get() ;
                pixel[1] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_BackgroundFilledValue-fit.Get()) * yIter.Get() ;
                pixel[2] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_BackgroundFilledValue-fit.Get()) * zIter.Get() ;
              }
              else
              {
                pixel.Fill(0.0);
              #if 0
                pixel[0] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_MovingImageBackground-m_FixedImageBackground) * xIter.Get() ;
                pixel[1] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_MovingImageBackground-m_FixedImageBackground) * yIter.Get() ;
                pixel[2] = 4.0 * m_MaximumUpdateStepLength * m_SimilarityWeight * (m_MovingImageBackground-m_FixedImageBackground) * zIter.Get() ;
              #endif
              }
              def12Iter.Set(pixel);
          }
...
*/
	...
	ImageRegionConstIterator<DeformationFieldFFTType> iter0(fft_filter->GetOutput(),fft_filter->GetOutput()->GetRequestedRegion());
   ...
   if(this->GetUseConsistentLandmark())
   {
        ...
        if(this->m_LandmarkWeight != 0)
        {
            ...
            IterationDeformationFieldType defIter(this->GetDeformationField(), this->GetDeformationField()->GetRequestedRegion());
	    ...
	    ImageRegionConstIterator<ComplexImageType> smoothIter(m_SmoothFilter, m_SmoothFilter->GetRequestedRegion());
            ImageRegionConstIterator<DeformationFieldFFTType> iter(fft_filter->GetOutput(),fft_filter->GetOutput()->GetRequestedRegion());
                ...
                ++iter, ++smoothIter, ++coeffsIter)
            {
                ...
                for(unsigned int i = 0; i < 3; i++)
                {
                    pixel1[i] = pixel1[i] - (m_LandmarkWeight * smoothIter.Get() * iter.Get()[i]) ; // * delta_normalizer);
                }
   ...
   typedef ICCIterativeInverseDeformationFieldImageFilter<TDeformationField, TDeformationField> InverseDeformationFieldImageType;
...
::ComputeMetric( void * gd)
{
  ...
  IterationImageType it_m(this->m_MovingImageWarper->GetOutput(), this->m_MovingImageWarper->GetOutput()->GetRequestedRegion());

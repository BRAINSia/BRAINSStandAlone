...
int AverageBrainGenerator(int argc, char *argv[])
{
  ...
  templateImage = itkUtil::OrientImage<ImageType>(templateImage,itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI); 
  //Read Directory
          ...
          if(subDir->Load(path.c_str()))
          {
             for(unsigned long j = 0; j < subDir->GetNumberOfFiles(); ++j )
             {
               if(itksys::SystemTools::StringEndsWith(subDir->GetFile(j),subName.c_str()))
               {
                 ...
                 typedef itk::AddImageFilter<DeformationFieldType, DeformationFieldType,DeformationFieldType> AddImageType;
  ...
  typedef itk::ICCIterativeInverseDeformationFieldImageFilter<DeformationFieldType, DeformationFieldType> InverseDeformationFieldImageType; 

...
//    this->Superclass::CopyInputToOutput();
      for(unsigned int i=0; i<2; i++)
	  {
		  ...
		  while( ! out.IsAtEnd() )
			{
			out.Value() =  static_cast<typename TDeformationField::PixelType>(in.Get());  // Supports input image adaptors only
...
//  float cutoff=0.0;
    ...
    for (myIterator.GoToBegin();!myIterator.IsAtEnd();++myIterator)
    {
        ...
        const unsigned int v=(static_cast<unsigned int>(HI[1])<ImageDims[1]/2)? static_cast<unsigned int>(HI[1]): ImageDims[1]-1-static_cast<unsigned int>(HI[1]);
        const unsigned int w=(static_cast<unsigned int>(HI[2])<ImageDims[2]/2)? static_cast<unsigned int>(HI[2]): ImageDims[2]-1-static_cast<unsigned int>(HI[2]);
...
void
ICCDeformableRegistrationFilter<TFixedImage,TMovingImage,TDeformationField>
::ThreadedComputeLinearElastic(DeformationFieldFFTPointer& coeff, float rho4delta_normalizer, const ThreadRegionType &regionToProcess, int)
{
...
::ComputeInverseConsistencyThreaderCallback(void * arg)
{
  ...
  if (threadId < total)
    {
    str->Filter->ThreadedComputeInverseConsistency(str->inverse0, str->inverse1, str->normalizer_InverseConsistency, splitRegion, threadId);
    }
...
void
ICCDeformableRegistrationFilter<TFixedImage,TMovingImage,TDeformationField>
::ThreadedComputeInverseConsistency(DeformationFieldFFTPointer& inv0, DeformationFieldFFTPointer& inv1, float normalizer, const ThreadRegionType &regionToProcess, int)
{

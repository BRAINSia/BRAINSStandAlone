  ...
  ::Execute()
{
           ...
           || fixedMMImage->GetMinimum()<(InputPixelType)itk::NumericTraits< unsigned char >::min())
    {
      ...
      if(this->GetOutDebug())
      {
         std::cout<<"Rescale the input fixed image from ["<<fixedMMImage->GetMinimum()<<", "<<fixedMMImage->GetMaximum()<<"] to: [0,255]"<<std::endl; 
      }
    }
           ...
           || movingMMImage->GetMinimum()<(InputPixelType)itk::NumericTraits< unsigned char >::min() ) 
    {
      ...
      if(this->GetOutDebug())
      {
         std::cout<<"Rescale the input moving image from ["<<movingMMImage->GetMinimum()<<", "<<movingMMImage->GetMaximum()<<"] to: [0,255]"<<std::endl;
      }
  ...
  if ( m_InitialTransformFilename != "" )
  {
    ...
    m_InitialDeformationField = itkUtil::OrientImage<TDeformationField>(m_InitialDeformationField, m_InputFixedImage->GetDirection());
  ...
  m_InputFixedImage = itkUtil::OrientImage<InputImageType>(m_InputFixedImage,itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
  m_InputMovingImage = itkUtil::OrientImage<InputImageType>(m_InputMovingImage,itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);

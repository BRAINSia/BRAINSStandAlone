...
int main(int argc, char * argv [])
{
  ...
  if (inputDiscreteVolume.size() == 0) { violated = true; std::cout << "  --inputDiscreteVolume Required! "  << std::endl; }
  ...
  LogisticRegression<PixelType> logisticRegressionWhiteVsCSF = LogisticRegression<PixelType>(featureCount, csfSampleCount+whiteMatterSampleCount);
  ...
  logisticRegressionProblemVeinousBloodVsAll.l = veinousBloodSampleCount + whiteMatterSampleCount; // Number of training samples
	logisticRegressionProblemVeinousBloodVsAll.y = Malloc(int,logisticRegressionProblemVeinousBloodVsAll.l);
	logisticRegressionProblemVeinousBloodVsAll.x = Malloc(struct feature_node *,logisticRegressionProblemVeinousBloodVsAll.l);
	struct feature_node *veinousBloodVsAllFeatureNodes;
  veinousBloodVsAllFeatureNodes = Malloc(struct feature_node,(featuresCount+bias)*logisticRegressionProblemVeinousBloodVsAll.l);
  ...
  logisticRegressionProblemWhiteVsGray.n = bias + featuresCount; // Number of features with bias term
  ...
  whiteVsGrayFeatureNodes = Malloc(struct feature_node,(featuresCount+bias)*logisticRegressionProblemWhiteVsGray.l);
  ...
  for ( imgItr.GoToBegin(); !imgItr.IsAtEnd(); ++imgItr)
    ...
    if (discretePixelValue == grayMatterDiscreteValue || discretePixelValue == basalGrayMatterDiscreteValue)
      {
        logisticRegressionProblemWhiteVsGray.x[whiteVsGraySampleCount] = &whiteVsGrayFeatureNodes[whiteVsGraySampleCount*(featuresCount+bias)];
        ...
        logisticRegressionProblemGrayVsCSF.x[csfVsGraySampleCount] = &GrayVsCSFFeatureNodes[csfVsGraySampleCount*(featuresCount+bias)];
        ...
        //logisticRegressionProblemVeinousBloodVsAll.x[veinousBloodVsAllSampleCount] = &veinousBloodVsAllFeatureNodes[veinousBloodVsAllSampleCount*(featuresCount+bias)];
    ...
    else if (discretePixelValue == whiteMatterDiscreteValue)
      {
        ...
        //logisticRegressionProblemWhiteVsCSF.x[whiteVsCSFSampleCount] = &whiteVsCSFFeatureNodes[whiteVsCSFSampleCount*(featuresCount+bias)];
...
//        logisticRegressionProblemVeinousBloodVsAll.y[veinousBloodVsAllSampleCount] = allStandInDiscreteValue;
  ...
  logisticRegressionModelVeinousBloodVsAll = train(&logisticRegressionProblemVeinousBloodVsAll, &logisticRegressionParameter);
  ...
  ImageType::Pointer outputImage = t1DuplicateImageFilter->GetOutput();
	...
	struct feature_node *sampleToPredict = (struct feature_node *) malloc((featuresCount+bias)*sizeof(struct feature_node));
...
//      }
    //else if( predictedProbabilityEstimatesWhiteVsCSF[0] > predictedProbabilityEstimatesWhiteVsCSF[1])
    else if( tempSample.GetLabelProbability(whiteMatterDiscreteValue) > tempSample.GetLabelProbability(csfDiscreteValue))
      {
      if ( predictedProbabilityEstimatesWhiteVsGray[0] < predictedProbabilityEstimatesWhiteVsGray[1] )
        ...
        if ( predictedProbabilityEstimatesGrayVsCSF[0] < predictedProbabilityEstimatesGrayVsCSF[1] )
          ...
          if ( predictedProbabilityEstimatesVeinousBloodVsAll[0] > predictedProbabilityEstimatesVeinousBloodVsAll[1] )
            ...
            {
            // white vs gray
            predictedOutputPixelValue = static_cast<ImageType::PixelType>(130 + (120 * predictedProbabilityEstimatesWhiteVsGray[0])) ;
            }
          }
        }
      else
      ...
      if ( predictedProbabilityEstimatesGrayVsCSF[0] < predictedProbabilityEstimatesGrayVsCSF[1] )
        ...
        if ( predictedProbabilityEstimatesWhiteVsGray[0] > predictedProbabilityEstimatesWhiteVsGray[1] )
          ...
          {
          // CSF Vs Gray
          predictedOutputPixelValue = static_cast<ImageType::PixelType>(10 + 120*predictedProbabilityEstimatesGrayVsCSF[0]) ;
          }
        }
      else
        {

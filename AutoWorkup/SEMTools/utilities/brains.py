...
class landmarksConstellationWeights(SlicerCommandLine):
...
class BRAINSTransformConvert(SlicerCommandLine):
...
class landmarksConstellationAligner(SlicerCommandLine):
...
class BRAINSEyeDetector(SlicerCommandLine):
...
class CleanUpOverlapLabels(SlicerCommandLine):
...
class insertMidACPCpoint(SlicerCommandLine):
    """title: MidACPC Landmark Insertion
...
class JointHistogram(SlicerCommandLine):
...
class ShuffleVectorsModule(SlicerCommandLine):
...
class ImageRegionPlotter(SlicerCommandLine):
    """title: Write Out Image Intensities

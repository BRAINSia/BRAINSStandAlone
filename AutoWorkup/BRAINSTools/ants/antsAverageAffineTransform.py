...
class AntsAverageAffineTransformInputSpec(ANTSCommandInputSpec):
...
class AntsAverageAffineTransformOutputSpec(TraitedSpec):
    affine_transform = File(exists=True, desc='average transform file')

class AntsAverageAffineTransform(ANTSCommand):
    """
    Examples
    --------
    >>>
    ...
    input_spec = AntsAverageAffineTransformInputSpec
    output_spec = AntsAverageAffineTransformOutputSpec

    def _format_arg(self, opt, spec, val):
        return super(AntsAverageAffineTransform, self)._format_arg(opt, spec, val)
